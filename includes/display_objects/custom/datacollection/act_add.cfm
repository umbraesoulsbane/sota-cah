...
<cfif $.content('subType') eq "myassets">
	<!--- // Workflow fields // --->
	<cfset request.fieldnames=listAppend(request.fieldnames,"submitter") />
		<cfset request.submitter = session.mura.userid >
<!---
	<cfset request.fieldnames=listAppend(request.fieldnames,"reviewers") />
		<cfset request.reviewers = "">
	<cfset request.fieldnames=listAppend(request.fieldnames,"approved") />
		<cfset request.approved = "">
--->

	<!--- // Validate Existance of Required Fields // --->
	<cfset reqFields = "assetid,uploadasset_attachment,assettitle,assettype,callout,assetdescription,submitter" />
	<cfset errFnd = false >
	<cfloop list="#reqFields#" index="rq">
		<cfif NOT ListContainsNoCase(request.fieldnames, rq) >
			<cfset errFnd = true >
		</cfif>	
	</cfloop>
	...
	<cfif errFnd >
		<cfset variables.acceptdata=0>
		<cfset variables.acceptError="Missing Fields">	
	</cfif>
</cfif>
...
<cfif variables.acceptdata>
	...
	<!--- // Asset Test and Save // --->
	<cfif $.content('subType') eq "myassets">
		<cfset variables.info=$.addAsset(structCopy(request))/>
	<cfelse>

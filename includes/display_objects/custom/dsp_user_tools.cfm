	...
	<div id="login" class="well clearfix">
		...
		<form action="<cfoutput>?nocache=1</cfoutput>" name="loginForm" method="post">
			...
			<cfif application.settingsManager.getSite(variables.$.event('siteID')).getExtranetPublicReg()>
				<p>#variables.$.rbKey('user.notregistered')# 
				<a href="https://www.shroudoftheavatar.com/wp-login.php?action=register">#variables.$.rbKey('user.signup')#</a>
				</p>
			</cfif>
		</form>
		</div>
...
<div id="svSessionTools" class="clearfix">
	<p id="welcome">#variables.$.rbKey('user.welcome')#, #HTMLEditFormat("#session.mura.fname#")#!</p>
</div>
</cfif>
